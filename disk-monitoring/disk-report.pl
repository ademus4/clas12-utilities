...
open(PASSWORD,"</home/baltzell/.mysql/diskmanager.pwd");
while(<PASSWORD>) { $password=$_; chomp $password; break; }

if (!$directory_label || !$limit || !$password) {
    print "disk-report.pl directory_label limit\n";
    exit 1;
}
...
$sql = "select sum($file_table.size) as sumsize, $file_table.uid from $file_table group by $file_table.uid order by sumsize desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
#
# This one seems not very useful for /work/clas12:
#
#print $q->h2("Greatest Average File Age by User"),
#              ######################################
#    "Size-weighted average age of all files owned by user\n";
#
#$sql = "select sum(($file_table.size*1.e-9)*(unix_timestamp(now())-unix_timestamp(atime)))/sum($file_table.size)*1.e9 as aveage, $file_table.uid from $file_table group by $file_table.uid order by aveage desc limit $limit;";
#make_query($dbh, \$sth);
#print "<table border>\n";
...
$sql = "select sum(($file_table.size*1.e-9)*(unix_timestamp(now())-unix_timestamp(atime))) as sumgby, $file_table.uid from $file_table group by $file_table.uid order by sumgby desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
$sql = "select $file_table.size, filename, atime, $file_table.uid, dirname from $dir_table, $file_table where $dir_table.id = dirId order by $file_table.size desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
$sql = "select atime, filename, $file_table.size, $file_table.uid, dirname from $dir_table, $file_table where $dir_table.id = dirId order by atime limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
$sql = "select ($file_table.size*1.e-9)*(unix_timestamp(now()) - unix_timestamp(atime)) as gby, filename, $file_table.size, atime, $dir_table.uid, dirname from $dir_table, $file_table where $dir_table.id = dirId order by gby desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
print $q->h2("Largest Directories"), "\nExcludes files in sub-directories\n";
              ###################
$sql = "select sum($file_table.size) as dirsize, dirname, $dir_table.uid from $dir_table, $file_table where dirId = $dir_table.id group by dirId order by dirsize desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
$sql = "select sum(($file_table.size*1.e-9)*(unix_timestamp(now()) - unix_timestamp(atime)))/sum($file_table.size)*1.e9 as aveage, dirname, $dir_table.uid from $dir_table, $file_table where $dir_table.id = dirId group by dirId order by aveage desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
...
$sql = "select sum(($file_table.size*1.e-9)*(unix_timestamp(now()) - unix_timestamp(atime))) as sumgby, dirname, $dir_table.uid from $dir_table, $file_table where $dir_table.id = dirId group by dirId order by sumgby desc limit $limit;";
make_query($dbh, \$sth);
print "<table border>\n";
